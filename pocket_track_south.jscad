...
function door() {
    var door = cube({size: [64.5, 45, 44.5]})
    door = translate([0,1.5,5], door)
    return door
}

function pocket() {
    var pocket = cube({size: [64.5, 48, 20]})
    return pocket
}

function stabilizer(){
    var stabilizer = cube({size:[20, 78, 4]})
    stabilizer = translate([0,-15,0], stabilizer)

    stabilizer = union(
        stabilizer,
        temp_corner([0,63.5,0]),
        temp_corner([64.5, 48, 0]),
